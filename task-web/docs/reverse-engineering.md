...
- Conclusion:
  ...
  - 이 가설을 재현하기 위한 live probe entry는 `examples/discovery/05-static-deob/probe-table-edit-surface.ts`다. 이 probe는 `ActionManager` method hook과 `.hcwo_selected_cell` DOM class, direct command state, pending insert queue, caret-local state를 키 시퀀스별로 함께 기록한다.
